execute as @s[type=player,tag=!has_costume] unless block ~ ~ ~ dark_oak_door run title @s actionbar ["",{"text":"[INFO]","color":"gold"},{"text":" Walk into a costume to wear it.","color":"none"}]
execute as @s[type=player,tag=has_costume] unless block ~ ~ ~ dark_oak_door run title @s actionbar ["",{"text":"[INFO]","color":"gold"},{"text":" Some costumes change their variant when you right click them.","color":"none"}]
