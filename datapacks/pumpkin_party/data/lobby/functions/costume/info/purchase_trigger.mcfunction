...
#Not enough money
execute unless score @s candies >= @e[type=minecraft:armor_stand,distance=..1.5,sort=nearest,limit=1,tag=Costume] const run tellraw @s ["",{"text":"You don't seem to have enough candy to unlock this costume.\nPlay more minigames to earn more candies.\n"}]
execute unless score @s candies >= @e[type=minecraft:armor_stand,distance=..1.5,sort=nearest,limit=1,tag=Costume] const run playsound minecraft:entity.villager.no master @s