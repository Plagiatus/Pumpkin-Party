execute if score #wd_phase wd_phases < #total wd_phases if entity @a[gamemode=adventure,tag=!wd_spectator] run function wd:play/bridge_back/start

execute if score #wd_phase wd_phases >= #total wd_phases run function wd:end_trigger
execute unless entity @a[gamemode=adventure,tag=!wd_spectator] run function wd:end_trigger
