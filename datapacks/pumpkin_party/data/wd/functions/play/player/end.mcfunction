...
execute as @a[gamemode=adventure,tag=!wd_spectator] if score @s wd_timer < #wd_total wd_timer run scoreboard players add @s wd_mistake 1

scoreboard players set #punish wd_timer 0
execute as @a[gamemode=adventure,tag=!wd_spectator,scores={wd_mistake=1..},limit=1] run scoreboard players set #punish wd_timer 1
