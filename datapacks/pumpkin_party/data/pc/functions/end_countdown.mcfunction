#Setup
execute if score pc_difficulty settings matches 2 run give @a minecraft:smooth_red_sandstone{ench:[],CanPlaceOn:["minecraft:black_terracotta","minecraft:smooth_red_sandstone"],HideFlags:31,Unbreakable:1,display:{Name:'"Pumpkin Filler"',Lore:['"Use this to fill in a block"','" you removed by mistake."','" "','"You only have a limited amount!"']}} 10
