...
execute if score #mseconds tmp matches ..9 if score #seconds tmp matches ..9 run tellraw @a [{"color": "red","text": ""},{"selector": "@s","color": "dark_red"}," got caught by the spider after ",{"score": {"name": "#minutes","objective": "tmp"}},":0",{"score": {"name": "#seconds","objective": "tmp"}},".0",{"score": {"name": "#mseconds","objective": "tmp"}}]
