...
execute positioned ^ ^ ^2 run kill @e[dy=0,tag=tot_doorMob]

#Witch effects
execute if entity @s[tag=tot_witch] run summon lightning_bolt ^ ^1 ^2
execute if entity @s[tag=tot_witch] run effect give @p[tag=tot_tricking,dy=0] minecraft:slowness 10 4
...
scoreboard players set @s tot_animation 0
